...
export function sellosPendientes(project, changes, git, { recalcular = false } = {}) {
  ...
  for (const c of candidatas) {
    ...
    const porContenido = posteriores.find((k) => contieneElCodigo(project.repoPath, k, c));
...
// ¿El código que registra esta entrada está en el archivo tal como quedó en
// ese commit? Se comparan líneas sueltas y no el bloque entero porque la
// captura recorta y une tramos con una marca; lo que importa es si ese código
// ya vivía ahí.
function contieneElCodigo(repoPath, commit, entrada) {
  for (const f of entrada.files || []) {
    if (!commit.archivos.includes(f.file)) return false;
    const contenido = leerArchivoEn(repoPath, commit.hash, f.file);
    if (contenido == null) return false;
    const presentes = new Set(lineasUtiles(contenido));
    const esperadas = lineasUtiles(String(f.after || '').split(CORTE_CAPTURA).join('\n'));
    if (esperadas.length < 2) continue;   // muy poco como para decidir
    const cuantas = esperadas.filter((l) => presentes.has(l)).length;
    if (cuantas / esperadas.length < 0.8) return false;
  }
  return true;
}
