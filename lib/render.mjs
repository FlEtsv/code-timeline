...
const TIMELINE_STYLE = `
  ...
  .mini-editor { margin-top: 12px; border: 1px solid var(--border); border-radius: 2px; overflow: hidden; }
  .mini-editor summary {
    cursor: pointer; list-style: none; display: flex; align-items: center; gap: 7px; padding: 8px 12px;
    font-family: 'JetBrains Mono', monospace; font-size: 10.5px; letter-spacing: .08em; text-transform: uppercase;
    color: var(--accent-ink); background: var(--accent-soft); user-select: none;
  }
  .mini-editor summary::-webkit-details-marker { display: none; }
  .mini-editor summary svg { width: 9px; height: 9px; transition: transform .15s ease; flex-shrink: 0; }
  .mini-editor[open] summary svg { transform: rotate(90deg); }
  .editor-body { max-height: 340px; overflow-y: auto; background: var(--surface); }
  .editor-loading, .editor-error { padding: 14px 16px; font-size: 12.5px; color: var(--ink-dim); font-style: italic; }
  .editor-error { color: var(--before); }
  .code-table { width: 100%; border-collapse: collapse; font-family: 'JetBrains Mono', monospace; font-size: 12px; line-height: 1.6; }
  .code-table tr.changed { background: var(--accent-soft); }
  .code-table tr.changed td.ln { color: var(--accent-ink); font-weight: 600; }
  .code-table td.ln {
    width: 1%; white-space: nowrap; text-align: right; padding: 0 12px 0 16px; color: var(--ink-faint);
    user-select: none; border-right: 1px solid var(--border); font-variant-numeric: tabular-nums;
  }
  .code-table td.src { padding: 0 16px 0 12px; white-space: pre; color: var(--ink); }
...
<div class="file-block">
  ...
  <details class="mini-editor" data-change="${esc(change.id)}" data-file-index="${idx}">
    <summary>
      ...
      Ver ${esc(f.file)} completo
    </summary>
    <div class="editor-body"><div class="editor-loading">Cargando…</div></div>
  </details>
</div>`;
}
...
(function () {
  ...
  document.querySelectorAll('.mini-editor').forEach(function (det) {
    det.addEventListener('toggle', function () {
      if (!det.open || det.dataset.loaded) return;
      var body = det.querySelector('.editor-body');
      var changeId = det.dataset.change;
      var fileIndex = det.dataset.fileIndex;
      fetch('/api/projects/' + encodeURIComponent(PROJECT_ID) + '/changes/' + encodeURIComponent(changeId) + '/files/' + fileIndex)
        .then(function (r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.text(); })
        .then(function (html) {
          body.innerHTML = html;
          det.dataset.loaded = '1';
          var changedRow = body.querySelector('tr.changed');
          if (changedRow && changedRow.scrollIntoView) changedRow.scrollIntoView({ block: 'center' });
        })
        .catch(function () {
          body.innerHTML = '<div class="editor-error">No se pudo cargar el archivo.</div>';
        });
    });
  });
