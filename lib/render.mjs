...
function controlesExplicacion() {
  return `<div class="explain-settings"><span class="explain-settings-label">Explicaciones futuras</span>
    ...
    <label class="explain-settings-label">Motor ${selectorMotor()}</label>
    <span class="explain-settings-label">Se aplica al generar o regenerar cualquier panel.</span></div>`;
}
...
function trustBlock(change) {
  const p = change.provenance || {};
  const codigo = p.code === 'git' ? 'Código capturado de Git' : 'Código aportado al registrar';
  const motivo = p.explanation === 'agent' ? 'Motivo declarado por el agente' : 'Motivo sin procedencia';
  const revision = change.verified ? 'Confirmado por una persona' : 'Pendiente de confirmación humana';
  return `<div class="proof"><div class="proof-head"><span class="proof-label">Confianza</span>
    <span class="pill">${esc(codigo)}</span><span class="pill">${esc(motivo)}</span><span class="pill">${esc(revision)}</span>
  </div></div>`;
}
