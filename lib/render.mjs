...
function codePanel(kind, code, language, opciones = {}) {
  ...
  const verOcultar = explicacion
    ? `<button type="button" class="btn-ver" aria-expanded="true">Ocultar explicación</button>`
    : '';
  ...
  const boton = puedePedir
    ? `<span class="explicar-controles">
        ${verOcultar}
        ${selectorNivel(kind)}
        ${selectorMotor(changeId, fileIndex, kind)}
        <button type="button" class="btn-explicar" data-id="${esc(changeId)}" data-file="${fileIndex}" data-lado="${kind}">`
      + `${explicacion ? 'Rehacer' : 'No entiendo esto'}</button>
      </span>`
    : '';

  return `<div class="panel panel-${kind}" data-panel="${kind}"${puedePedir ? ` data-id="${esc(changeId)}" data-file="${fileIndex}"` : ''}>
...
function selectorNivel(kind) {
  const opciones = Object.entries(NIVELES).map(([id, n]) => (
    `<option value="${esc(id)}"${id === 'normal' ? ' selected' : ''}>${esc(n.nombre)}</option>`
  )).join('');
  return `<select class="sel-nivel" data-lado="${kind}" title="Cuánto detalle">${opciones}</select>`;
}
...
function selectorMotor(changeId, fileIndex, kind) {
  ...
  return `<select class="sel-motor" data-id="${esc(changeId)}" data-file="${fileIndex}" data-lado="${kind}"
    title="Quién lo explica">${opciones}</select>`;
}
...
(function () {
  ...
  function ponerBotonVer(panel, visible) {
    ...
    if (!b) {
      ...
      b.addEventListener('click', function () { plegar(panel, !panel.classList.contains('plegada')); });
    }
    b.textContent = visible ? 'Ocultar explicación' : 'Ver explicación';
  ...
  document.querySelectorAll('.btn-explicar').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      var sel = panel.querySelector('.sel-motor');
      var selN = panel.querySelector('.sel-nivel');
