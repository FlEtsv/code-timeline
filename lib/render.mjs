...
const TIMELINE_STYLE = `
  ...
  /* --- El cockpit de ramas: una franja sobre el timeline. Solo aparece si hay
     más de una rama; pulsar una filtra el historial de abajo. Todo son enlaces
     (?branch=...): funciona sin JS. --- */
  .branchstrip { border-bottom: 2px solid var(--ink); padding: 15px 0 0; margin-top: 2px; }
  .branchstrip-head { display: flex; align-items: baseline; gap: 10px; margin-bottom: 10px; font-family: 'JetBrains Mono', monospace; font-size: 11px; letter-spacing: .1em; text-transform: uppercase; color: var(--ink-dim); }
  .branchstrip-head b { color: var(--ink); }
  .branchstrip-head .fetch { margin-left: auto; text-transform: none; letter-spacing: 0; border: 1px solid var(--border); border-radius: 2px; padding: 3px 9px; }
  .branchstrip-head .fetch:hover { border-color: var(--accent); color: var(--accent-ink); }
  .branches { display: flex; gap: 9px; overflow-x: auto; padding-bottom: 13px; }
  .bcard { flex: 0 0 auto; min-width: 200px; max-width: 300px; border: 1px solid var(--border); border-radius: 3px; background: var(--surface); padding: 10px 12px 11px; text-decoration: none; color: inherit; }
  .bcard:hover { border-color: var(--ink-dim); }
  .bcard.active { border-color: var(--accent); box-shadow: inset 0 -2px 0 var(--accent); }
  .bcard.remote { border-style: dashed; opacity: .72; }
  .bcard .bn { font-family: 'JetBrains Mono', monospace; font-size: 12.5px; color: var(--ink); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .bcard.active .bn { color: var(--accent-ink); font-weight: 600; }
  .bcard .brow { display: flex; align-items: center; gap: 11px; margin-top: 7px; font-family: 'JetBrains Mono', monospace; font-size: 11px; color: var(--ink-dim); font-variant-numeric: tabular-nums; }
  .bcard .ab .up { color: var(--after); } .bcard .ab .dn { color: var(--ink-faint); }
  .bcard .mg { color: var(--after); }
  .bcard .gap { margin-left: auto; font-size: 10px; border: 1px solid var(--before-line); color: var(--before); background: var(--before-hatch); border-radius: 2px; padding: 2px 6px; }
  .bcard .gap.zero { border-color: var(--border); color: var(--ink-faint); background: transparent; }
  .filterbar { display: flex; align-items: center; gap: 11px; margin: 15px 0 2px; padding: 8px 12px; background: var(--accent-soft); border: 1px solid var(--accent); border-radius: 3px; font-family: 'JetBrains Mono', monospace; font-size: 12px; color: var(--accent-ink); flex-wrap: wrap; }
  .filterbar b { font-weight: 600; }
  .filterbar .sep { color: var(--accent); opacity: .5; }
  .filterbar .clear { margin-left: auto; border: 1px solid var(--accent); border-radius: 2px; padding: 3px 9px; color: var(--accent-ink); text-decoration: none; }
  .filterbar .clear:hover { background: var(--accent); color: var(--surface); }
  ...
  .chip-by { border-style: dashed; }
  .chip-branch { border-color: var(--accent); color: var(--accent); }
...
function codePanel(kind, code, language) {
  ...
  return `<div class="panel panel-${kind}"><div class="panel-label">${label}</div><pre><code>${highlight(code, language)}</code></pre></div>`;
}
...
<div class="file-block">
  ...
  <div class="diff-grid">
    ${codePanel('before', f.before, f.language)}
    ${codePanel('after', f.after, f.language)}
  </div>
</div>`;
}
...
function cardHead(change) {
  ...
  // Contexto de equipo/rama: quién registró y en qué rama. Contenido de
  // usuario, así que va escapado. Slice 1 lo enseña discreto; el navegador de
  // ramas de Slice 2 se apoya en el mismo `data-branch` de la <article>.
  const meta = [
    change.by ? `<span class="chip chip-by">${esc(change.by)}</span>` : '',
    change.branch ? `<span class="chip chip-branch">${esc(change.branch)}</span>` : '',
  ].join('');

  if (change.status === 'proposal') {
    return `<div class="card-head">${chip}${meta}<span class="badge badge-proposal">Propuesta</span></div>`;
  }
  if (change.status === 'accepted') {
    return `<div class="card-head">${chip}${meta}<span class="badge badge-approved">Aceptada · falta aplicarla</span></div>`;
  }
  if (change.status === 'rejected') {
    return `<div class="card-head">${chip}${meta}<span class="badge badge-rejected">Descartada</span></div>`;
  }
...
<article class="${cls}" data-id="${esc(change.id)}" data-status="${status}" data-branch="${esc(change.branch || '')}">
...
// La franja de ramas. `cockpit` = branchReport(...) + `branch` (el filtro
// activo). Solo se pinta si hay ramas además de la principal. Las tarjetas son
// enlaces `?branch=`, así que filtra sin una línea de JS.
function branchStrip(cockpit, projectId) {
  if (!cockpit || !cockpit.ramas || !cockpit.ramas.length) return '';
  const base = `/p/${encodeURIComponent(projectId)}`;
  const card = (r) => {
    const activa = r.nombre === cockpit.branch;
    const cls = ['bcard', activa && 'active', r.remota && 'remote'].filter(Boolean).join(' ');
    const href = r.remota ? '' : ` href="${base}?branch=${encodeURIComponent(r.nombre)}"`;
    const pos = r.remota
      ? (r.fusionada ? '<span class="mg">fusionada</span>' : '<span>remota</span>')
      : `<span class="ab"><span class="up">▲${r.adelante ?? '?'}</span> <span class="dn">▼${r.atras ?? '?'}</span></span>`
        + (r.fusionada ? ' <span class="mg">fusionada</span>' : '');
    const huecos = r.sinRegistrar == null ? ''
      : r.sinRegistrar === 0 ? '<span class="gap zero">al día</span>'
      : `<span class="gap">${r.sinRegistrar} sin registrar</span>`;
    const tag = r.remota ? 'div' : 'a';
    return `<${tag} class="${cls}"${href}>
      <div class="bn">${esc(r.nombre)}</div>
      <div class="brow">${pos}<span>${r.entradas} entr</span>${huecos}</div>
    </${tag}>`;
  };
  const filtro = cockpit.branch ? `
  <div class="filterbar">
    <span>Rama</span><b>${esc(cockpit.branch)}</b>
    <span class="sep">·</span><span>${cockpit.ramas.find((r) => r.nombre === cockpit.branch)?.entradas ?? 0} entradas</span>
    <a class="clear" href="${base}">✕ ver todo</a>
  </div>` : '';
  return `<section class="branchstrip">
    <div class="branchstrip-head">Ramas · principal <b>${esc(cockpit.principal || '—')}</b>
      <a class="fetch" href="${base}?fetch=1${cockpit.branch ? `&branch=${encodeURIComponent(cockpit.branch)}` : ''}">↓ traer del remoto</a>
    </div>
    <div class="branches">${cockpit.ramas.map(card).join('')}</div>
  </section>${filtro}`;
}

export function renderTimelineHtml(project, changes, qa, copiloto, token, cockpit) {
  // Filtro por rama del cockpit: acota SOLO el historial (las propuestas y lo
  // pendiente no cuelgan de una rama).
  if (cockpit && cockpit.branch) {
    changes = changes.filter((c) => (c.status || 'change') !== 'change' || c.branch === cockpit.branch);
  }
...
<div class="page">
  ...
  ${branchStrip(cockpit, project.id)}
        ...
        <div class="sidebar-meta">
          ...
          ${status === 'change' ? '' : `<div class="sidebar-proposed">
            ...
            ${files.map((f) => `<div class="file-tag" style="margin-bottom:7px"><b>${esc(f.file)}</b></div>
              ${codePanel('before', f.before, f.language)}${codePanel('after', f.after, f.language)}`).join('')}
          </div>`}
