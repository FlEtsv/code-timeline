...
<div class="page">
  ...
  ${pending.length ? '<div class="section-head"><h2>Historial</h2><span class="section-sub">Cambios ya aplicados</span></div>' : ''}
  <div class="timeline">
...
const DETAIL_STYLE = `
  ...
  .tree-change {
    display: flex; align-items: center; gap: 7px; padding: 5px 9px; margin: 1px 0;
  ...
  .tree-change-title { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .tree-dot {
    width: 9px; height: 9px; border-radius: 50%; border: 1.5px solid var(--border); flex-shrink: 0;
  ...
  .tree-change:has(.tree-dot.done):not(.current) { opacity: .58; }
  ...
  .tree-change.tree-rejected { opacity: .5; text-decoration: line-through; text-decoration-color: var(--ink-faint); }
...
<div class="detail-shell">
  ...
  ${status === 'change' ? '' : `<div class="detail-banner">
    <strong>${status === 'proposal' ? 'Esto todavía no está en el código.' : 'Propuesta descartada.'}</strong>
