...
import { MOTORES } from './explicar.mjs';
...
export function bloqueExplicacion(e) {
  ...
  return `<div class="explicacion">
    ...
    <p class="explicacion-pie">Explicado por ${esc(e.modelo || 'Claude')}${e.fecha ? ' · ' + esc(fmtDateShort(e.fecha)) : ''}${esc(gastoTexto(e.gasto))}. No toca el código: es una capa de lectura.</p>
  </div>`;
}
...
(function () {
  ...
  document.querySelectorAll('.panel').forEach(marcarQueTienen);
    ...
    btn.addEventListener('click', function () {
      ...
      fetch('/api/projects/' + encodeURIComponent(PROJECT_ID) + '/changes/'
        ...
        body: JSON.stringify({
          ...
          motor: elegido[0], modelo: elegido.slice(1).join(':'),
        }),
      })
        .then(function (r) { return r.json(); })
        .then(function (d) {
