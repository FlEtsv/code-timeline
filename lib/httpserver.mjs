...
function fileTableHtml(project, change, f) {
  try {
    ...
    return renderFileTable(content, f.lineStart, f.lineEnd);
  } catch (err) {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && cMatch) {
        ...
        const firstFileHtml = firstFile ? fileTableHtml(project, change, firstFile) : '<div class="editor-error">Este cambio no tiene archivos registrados.</div>';
        return send(res, 200, renderChangeDetailHtml(project, changes, index, firstFileHtml));
      }
