...
import { listProjects, getProject, listChanges, updateChange, decideProposal, exportProject, lastQaRun } from './store.mjs';
...
// código fuente del usuario, sus notas y una API de escritura sin ninguna
// autenticación. Escuchar en 0.0.0.0 lo deja a la vista de cualquiera que
// comparta la wifi. Salir a la red es una decisión del usuario (--host).
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && pMatch) {
        ...
        return send(res, 200, renderTimelineHtml(project, changes, lastQaRun(pMatch[1])));
      }
