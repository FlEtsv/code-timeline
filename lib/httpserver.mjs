...
import { listProjects, getProject, listChanges, updateChange, decideProposal, exportProject, lastQaRun, setExplicacion } from './store.mjs';
import { renderTimelineHtml, renderIndexHtml, renderChangeDetailHtml, renderFileTable, esc, bloqueExplicacion } from './render.mjs';
import { renderMarkdown } from './markdown.mjs';
import { aconsejar, cuerpoPr } from './consejo.mjs';
import { explicar } from './explicar.mjs';
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && pMatch) {
        ...
        return send(res, 200, renderTimelineHtml(project, changes, lastQaRun(pMatch[1]), aconsejar(project, changes), TOKEN));
      }
      ...
      // Pedir la explicación de un trozo de código. Escribe —guarda la
      // explicación en la entrada— así que pide token como las demás.
      //
      // A diferencia de las acciones, esta espera la respuesta: tarda segundos,
      // no minutos, y devolver un trabajo que hay que ir preguntando sería más
      // aparato del que hace falta.
      const explicarMatch = url.pathname.match(/^\/api\/projects\/([^/]+)\/changes\/([^/]+)\/explicar$/);
      if (req.method === 'POST' && explicarMatch) {
        if (!autorizado(req)) return sendJson(res, 403, { error: 'Petición no autorizada.' });
        const [, projectId, changeId] = explicarMatch;
        const body = await readJsonBody(req);
        const lado = body.lado === 'before' ? 'before' : 'after';
        const change = listChanges(projectId).find((c) => c.id === changeId);
        if (!change) return sendJson(res, 404, { error: 'No existe esa entrada.' });
        const f = (change.files || [])[Number(body.fileIndex)];
        if (!f) return sendJson(res, 404, { error: 'No existe ese archivo en la entrada.' });
        const codigo = f[lado];
        if (codigo == null) return sendJson(res, 400, { error: 'Ese panel no tiene código que explicar.' });

        const explicacion = await explicar(codigo, {
          file: f.file,
          language: f.language,
          motor: body.motor,
          modelo: body.modelo,
          nivel: body.nivel,
          // El porqué que se registró ayuda a explicar QUÉ hace: da el
          // propósito, que no siempre se deduce del código solo.
          contexto: change.explanation,
        });
        setExplicacion(projectId, changeId, { fileIndex: Number(body.fileIndex), lado, explicacion });
        return sendJson(res, 200, { explicacion, html: bloqueExplicacion(explicacion) });
      }
