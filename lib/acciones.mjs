...
// La web puede delegar una propuesta aceptada en Claude Code o en Codex.
// Claude sigue siendo el valor histórico; CODE_TIMELINE_AGENT=codex cambia
// solo este lanzador, no el formato MCP ni el historial.
const AGENTE = (process.env.CODE_TIMELINE_AGENT || 'claude').toLowerCase();
...
export function comandoAgente({ prompt, herramientas, agente = AGENTE } = {}) {
  if (agente === 'claude') return {
    binario: process.env.CODE_TIMELINE_CLAUDE || 'claude',
    args: ['-p', prompt, '--allowedTools', herramientas.join(' ')],
  };
  if (agente === 'codex') return {
    binario: process.env.CODE_TIMELINE_CODEX || 'codex',
    // Ephemeral evita llenar el historial de sesiones con trabajos nacidos de
    // un botón. El MCP se confía una vez en config.toml; el sandbox de Codex
    // sigue siendo workspace-write y no hace falta relajar las aprobaciones de
    // las demás herramientas del agente.
    args: ['exec', '--ephemeral', '--color', 'never', prompt],
  };
  throw new Error(`Agente no admitido: "${agente}". Usa "claude" o "codex".`);
}

function lanzarAgente(t, { cwd, prompt, herramientas }) {
  let comando;
  try {
    comando = comandoAgente({ prompt, herramientas });
  } catch (err) {
    t.estado = 'fallo';
    t.error = err.message;
  ...
  const hijo = spawn(comando.binario, comando.args, { cwd, stdio: ['ignore', 'pipe', 'pipe'] });
  ...
  hijo.on('error', (err) => {
    clearTimeout(reloj);
    t.estado = 'fallo';
    t.error = `No se pudo ejecutar "${comando.binario}": ${err.message}`;
    t.terminado = new Date().toISOString();
  });
  ...
  hijo.on('close', (codigo) => {
    ...
    if (codigo !== 0 && !t.error) t.error = `${AGENTE === 'codex' ? 'Codex' : 'Claude'} terminó con código ${codigo}.`;
    t.terminado = new Date().toISOString();
  });
