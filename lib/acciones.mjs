...
export function aplicarPropuesta(projectId, changeId) {
  ...
  const archivos = (propuesta.files || []).map((f) => `- ${f.file}${f.lineStart ? `:${f.lineStart}` : ''}`).join('\n');
  const prompt = [
    ...
    '',
    '## Archivos que toca',
    archivos || '(ninguno indicado)',
    '',
    '## El código propuesto',
    ...(propuesta.files || []).map((f) => [
      `### ${f.file}`,
      f.before == null ? '(código nuevo, no existía)' : '```\n' + f.before + '\n```',
      'se convierte en:',
      '```\n' + f.after + '\n```',
    ].join('\n')),
    '',
    ...
    '4. Comprueba que no rompes nada: si el proyecto tiene pruebas, pásalas.',
    '5. Al terminar llama a mark_applied con projectId "' + projectId + '", changeId "' + changeId + '" y los archivos REALES que escribiste.',
    '6. Si escribiste o ejecutaste una prueba, regístrala con set_test. Si falla, dilo con status "failing".',
    '',
