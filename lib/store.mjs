...
// Aceptar una propuesta la convierte en un cambio del historial: es el mismo
// registro, no una copia, así que la nota que dejaste al revisarla y su
// antes/después siguen ahí. Rechazarla la archiva con tu motivo — no se borra:
// saber qué se descartó y por qué es la mitad del valor de haberlo propuesto.
export function decideProposal(id, changeId, { decision, note } = {}) {
  ...
  changes[idx] = {
    ...current,
    status: decision === 'accept' ? 'change' : 'rejected',
