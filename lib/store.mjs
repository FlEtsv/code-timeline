...
import {
  cambiosSinRegistrar, autorDeGit, leerSha, ramaActual, ramaPrincipal,
  ramas as ramasGit, commitsDeRama, traer, estadoDeRamaEntrada,
} from './git.mjs';
...
function readJson(path, fallback, reparar = true) {
...
// El directorio de datos de un proyecto (donde viven changes.json / qa.json).
// Lo usa `code-timeline team ...` para llevarlo como su propio repo git privado.
export function projectDataDir(idORuta) {
  return projectDir(resolverProyecto(idORuta).id);
}

// True si ese directorio ya es un repo git (lo puso `team init`).
export function esDeEquipo(idORuta) {
  return existsSync(join(projectDataDir(idORuta), '.git'));
}
...
function normalize(change) {
  return {
    ...
    decisionNote: '',
    fromProposal: false,
    // Aditivos: un data/ viejo no los trae y no pasa nada. `by` = quién registró
    // (identidad git); `branch`/`baseSha` anclan la entrada a la rama y al punto
    // exacto en que se hizo; `updatedAt` lo usa el merge driver del sync de equipo.
    by: '',
    branch: '',
    baseSha: '',
    ...change,
    updatedAt: change.updatedAt || change.date || null,
...
export function resumirCambio(c, { explicacion = 240 } = {}) {
  ...
  return {
    ...
    verified: c.verified,
    commit: c.commit,
    // Contexto de equipo/rama: barato y orienta al leer el historial.
    ...(c.by ? { by: c.by } : {}),
...
function addEntryLocked(id, entry, status) {
  ...
  const ahora = entry.date || new Date().toISOString();
  const change = {
    id: randomUUID(),
    status,
    date: ahora,
    commit: entry.commit || null,
    // Quién y desde dónde. `by` se pasa o se toma del `git config` del repo;
    // `branch`/`baseSha` se leen del repo en el momento de registrar, para poder
    // agrupar el historial por rama y saber después si esa rama se fusionó.
    by: entry.by || autorDeGit(proyecto.repoPath),
    branch: entry.branch || ramaActual(proyecto.repoPath) || '',
    baseSha: entry.baseSha || leerSha(proyecto.repoPath),
    updatedAt: ahora,
...
export function updateChange(id, changeId, patch) {
  ...
  return withLock(changesFile(id), () => {
    ...
    changes[idx].updatedAt = new Date().toISOString();
    writeJson(changesFile(id), { changes });
    return changes[idx];
  });
}
...
export function setTest(id, changeId, { status, command, note } = {}) {
  ...
  return withLock(changesFile(id), () => {
    ...
    changes[idx] = { ...changes[idx], test: next, updatedAt: new Date().toISOString() };
    writeJson(changesFile(id), { changes });
    return changes[idx];
  });
}
...
function resolverProyecto(idORuta) {
  const { projects } = readJson(PROJECTS_FILE, { projects: [] }, false);
  const project = projects.find((p) => p.id === idORuta) ||
    projects.find((p) => p.repoPath && rutaReal(p.repoPath) === rutaReal(idORuta));
  if (!project) throw new Error(`No hay ningún proyecto vinculado para ${idORuta}.`);
  return project;
}

export function syncReport(idORuta, { branch } = {}) {
  const project = resolverProyecto(idORuta);
  const { changes } = readJson(changesFile(project.id), { changes: [] }, false);
  return { projectId: project.id, ...cambiosSinRegistrar(project.repoPath, changes, { branch }) };
}

// El cockpit de ramas: para cada rama del repo, cuántas entradas del historial
// la nombran y cuántos commits suyos no están registrados. `fetch` trae del
// remoto primero (opt-in: mueve refs, así que solo si se pide).
export function branchReport(idORuta, { fetch = false } = {}) {
  const project = resolverProyecto(idORuta);
  const repoPath = project.repoPath;
  if (fetch) traer(repoPath);
  const { changes } = readJson(changesFile(project.id), { changes: [] }, false);
  const aplicadas = changes.map(normalize).filter((c) => c.status === 'change');
  const principal = ramaPrincipal(repoPath);
  const lista = ramasGit(repoPath, { principal });

  const filas = lista.map((r) => {
    const local = r.nombre.replace(/^origin\//, '');
    const entradas = aplicadas.filter((c) => c.branch === local).length;
    let sinRegistrar = null;
    if (!r.remota) {
      try {
        sinRegistrar = cambiosSinRegistrar(repoPath, changes, { branch: r.nombre }).count;
      } catch { sinRegistrar = null; }
    }
    return { ...r, entradas, sinRegistrar };
  });
  return {
    projectId: project.id,
    principal,
    ramaActual: ramaActual(repoPath),
    ramas: filas,
  };
}

// El estado de cada entrada respecto a su rama (viva / fusionada / huérfana).
export function estadosDeRama(idORuta) {
  const project = resolverProyecto(idORuta);
  const { changes } = readJson(changesFile(project.id), { changes: [] }, false);
  const out = {};
  for (const c of changes.map(normalize)) {
    out[c.id] = estadoDeRamaEntrada(project.repoPath, c);
  }
  return out;
}
