...
function writeJson(path, data, { backup = true } = {}) {
...
function versionedHistoryFile(project) {
  return join(project.repoPath, '.code-timeline', 'history.json');
}

function versionedIndexFile(project) {
  return join(project.repoPath, '.code-timeline', 'index.json');
}

function writeVersioned(project, changes) {
  const root = join(project.repoPath, '.code-timeline');
  const legacy = versionedHistoryFile(project);
  const index = versionedIndexFile(project);
  if (existsSync(legacy) && !existsSync(index)) {
    renameSync(legacy, join(root, 'history.v1.json'));
  }
  for (const change of changes) {
    writeJson(join(root, 'entries', `${change.id}.json`), {
      format: 'code-timeline/entry-v2',
      projectId: project.id,
      change,
    }, { backup: false });
  }
  writeJson(index, {
    format: 'code-timeline/versioned-v2',
    project: { id: project.id, name: project.name },
    entries: changes.map((c) => ({ id: c.id, date: c.date, path: `entries/${c.id}.json` })),
  }, { backup: false });
}

export function readVersionedBundle(repoPath) {
  const root = resolve(repoPath, '.code-timeline');
  const index = join(root, 'index.json');
  if (existsSync(index)) {
    const data = readJson(index, null);
    if (!data || data.format !== 'code-timeline/versioned-v2') throw new Error('Índice versionado desconocido.');
    const changes = (data.entries || []).map((entry) => {
      const target = resolve(root, entry.path || '');
      if (!target.startsWith(`${root}${sep}`)) throw new Error(`Ruta de entrada inválida: ${entry.path}`);
      const item = readJson(target, null);
      if (!item || item.format !== 'code-timeline/entry-v2' || !item.change) throw new Error(`Entrada versionada inválida: ${entry.path}`);
      return normalize(item.change);
    });
    return { format: EXPORT_FORMAT, project: data.project || {}, changes };
  }
  const legacy = join(root, 'history.json');
  if (!existsSync(legacy)) return null;
  const data = readJson(legacy, null);
  if (!data || data.format !== 'code-timeline/versioned-v1') throw new Error('Historial versionado desconocido.');
  return { format: EXPORT_FORMAT, project: data.project || {}, changes: (data.changes || []).map(normalize) };
}

function writeChanges(id, data) {
  writeJson(changesFile(id), data);
  const project = getProject(id);
  if (project.storageMode === 'versioned') {
    writeVersioned(project, data.changes);
  }
}
...
export function getProject(idORuta) {
  ...
  if (isAbsolute(clave) || clave.includes('/') || clave.includes('\\')) {
...
export function createProject({ name, repoPath, githubRemote, storageMode = 'private' }) {
  ensureDataDir();
  ...
  return withLock(PROJECTS_FILE, () => {
    ...
    const project = {
      ...
      storageMode: storageMode === 'versioned' ? 'versioned' : 'private',
...
export function syncVersioned(id) {
  const project = getProject(id);
  if (project.storageMode !== 'versioned') return null;
  const data = readJson(changesFile(project.id), { changes: [] });
  writeVersioned(project, data.changes);
  return versionedIndexFile(project);
}
...
function normalize(change) {
  return {
    ...
    decisionNote: '',
    fromProposal: false,
    provenance: { explanation: 'agent', code: 'unknown', humanVerified: false },
    ...change,
...
// Buscar en el historial sin traérselo entero.
//
// Es lo que faltaba para poder preguntar "¿dónde tocamos el candado entre
// procesos?". Un grep no sirve: el porqué está en prosa y no contiene esas
// palabras literales necesariamente. Y traerse las 151 entradas para mirarlas
// cuesta miles de tokens cada vez.
//
// Sin dependencias ni índice: se puntúa cada entrada por cuántos términos de
// la consulta aparecen y dónde. El título pesa más que el porqué, y el porqué
// más que la ruta de un archivo, porque quien busca suele recordar de qué iba
// antes que en qué fichero fue.
export function buscar(id, consulta, { limit = 5 } = {}) {
  const terminos = String(consulta || '')
    .toLowerCase()
    .split(/[^\p{L}\p{N}_]+/u)
    .filter((t) => t.length > 2);
  ...
  const puntuar = (c) => {
    const titulo = String(c.title || '').toLowerCase();
    const porque = String(c.explanation || '').toLowerCase();
    const rutas = (c.files || []).map((f) => f.file).join(' ').toLowerCase();
    const unidad = `${(c.unit || {}).type || ''} ${(c.unit || {}).name || ''}`.toLowerCase();

    let punto = 0;
    for (const t of terminos) {
      if (titulo.includes(t)) punto += 10;
      if (unidad.includes(t)) punto += 6;
      if (rutas.includes(t)) punto += 3;
      // El porqué puede repetir el término muchas veces; se cuenta con tope
      // para que un texto largo no gane solo por ser largo.
      const veces = porque.split(t).length - 1;
      if (veces) punto += Math.min(veces, 3) * 2;
    }
    return punto;
  };

  return listChanges(id)
    .map((c) => ({ c, punto: puntuar(c) }))
    .filter((x) => x.punto > 0)
    .sort((a, b) => b.punto - a.punto || b.c.date.localeCompare(a.c.date))
    .slice(0, limit)
    .map(({ c, punto }) => ({
      id: c.id,
      date: c.date,
      ...
      status: c.status,
      commit: c.commit,
      files: (c.files || []).map((f) => f.file),
      // Un trozo del porqué alrededor de donde casa, no los primeros 240
      // caracteres: lo que se busca suele estar en medio.
      porque: extracto(c.explanation, terminos),
      punto,
    }));
}

function extracto(texto, terminos, ancho = 200) {
  const t = String(texto || '');
  const bajo = t.toLowerCase();
  let donde = -1;
  for (const term of terminos) {
    const i = bajo.indexOf(term);
    if (i !== -1 && (donde === -1 || i < donde)) donde = i;
  }
  if (donde === -1) return t.slice(0, ancho) + (t.length > ancho ? '…' : '');
  const desde = Math.max(0, donde - Math.floor(ancho / 3));
  return (desde ? '…' : '') + t.slice(desde, desde + ancho) + (desde + ancho < t.length ? '…' : '');
}
...
function normalizeFiles(files) {
  return (files || []).map((f) => ({
    file: f.file,
    fileId: f.fileId || createHash('sha256').update(String(f.file || '')).digest('hex').slice(0, 12),
    ...
    after: f.after,
    // Identidad estable del trozo, independiente del id aleatorio de la
    // entrada y de su posición en el array. Sobrevive a exportar/importar y
    // permite enlazar una revisión con el mismo cambio de contenido.
    hunkId: f.hunkId || createHash('sha256').update([
      f.file || '', f.lineStart ?? '', f.lineEnd ?? '', f.before ?? '', f.after ?? '',
...
function addEntryLocked(id, entry, status) {
  ...
  const change = {
    ...
    provenance: {
      explanation: 'agent',
      code: files.every((f) => f.capturado) ? 'git' : 'provided',
      humanVerified: false,
    },
...
export function updateChange(id, changeId, patch) {
  ...
  return withLock(changesFile(id), () => {
    ...
    if ('verified' in patch) {
      changes[idx].verified = !!patch.verified;
      changes[idx].provenance = {
        explanation: 'agent', code: 'unknown', ...(changes[idx].provenance || {}),
        humanVerified: !!patch.verified,
      };
    }
...
export function stampCommits(id, sellos) {
  ...
  return withLock(changesFile(id), () => {
    ...
    for (const { changeId, commit, corrige, exacto } of sellos) {
      ...
      changes[idx].anchorId = createHash('sha256').update([
        String(commit), changes[idx].id, ...(changes[idx].files || []).map((f) => f.hunkId || ''),
      ].join('\0')).digest('hex').slice(0, 20);
      changes[idx].files = (changes[idx].files || []).map((f) => ({
        ...f,
        anchorId: createHash('sha256').update([String(commit), f.fileId || f.file, f.hunkId || ''].join('\0')).digest('hex').slice(0, 20),
      }));
      hechos.push({ changeId, commit: String(commit), title: changes[idx].title });
    }
    if (hechos.length) writeChanges(id, { changes });
...
export function importProject(bundle, { mode = 'new', targetId, repoPath } = {}) {
  ...
  writeChanges(project.id, { changes: incoming });
...
function recallsFile(id) {
  return join(projectDir(id), 'recalls.json');
}
...
export function recordRecall(id, changeId, { avoided = false, minutes = 0, note = '' } = {}) {
  const change = getChange(id, changeId);
  return withLock(recallsFile(id), () => {
    const { recalls } = readJson(recallsFile(id), { recalls: [] });
    const recall = {
      id: randomUUID(), changeId, at: new Date().toISOString(),
      entryAgeHours: Math.max(0, Math.round((Date.now() - Date.parse(change.date)) / 3600000)),
      avoided: Boolean(avoided), minutes: Math.max(0, Number(minutes) || 0), note: String(note),
    };
    recalls.push(recall);
    writeJson(recallsFile(id), { recalls });
    return recall;
  });
}

export function recallStats(id) {
  getProject(id);
  const { recalls } = readJson(recallsFile(id), { recalls: [] });
  return {
    recoveries: recalls.length,
    oldRecoveries: recalls.filter((r) => r.entryAgeHours >= 24).length,
    avoided: recalls.filter((r) => r.avoided).length,
    minutesSaved: recalls.reduce((n, r) => n + (r.avoided ? r.minutes : 0), 0),
  };
}
