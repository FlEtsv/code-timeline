...
function inline(texto) {
  ...
  // devuelven al final.
  const codigos = [];
  // Un NUL en el propio texto podría imitar la marca y traerse el contenido de
  // otro tramo, o dejar un "undefined" a la vista. No pinta nada en HTML, así
  // que se quita antes de nada.
  let s = esc(texto).replace(/\u0000/g, '').replace(/`([^`\n]+)`/g, (_, code) => {
