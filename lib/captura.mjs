...
// Si este archivo es el destino de un renombrado, devuelve el nombre que tenía
// antes. git solo lo dice mirando el árbol completo, así que se pregunta con
// --name-status sin restringir la ruta.
function nombreAnterior(repoPath, archivo) {
  for (const contra of ['HEAD', 'HEAD~1']) {
    const salida = git(repoPath, ['diff', '--name-status', '-M', '--no-color', contra]);
    if (!salida) continue;
    for (const linea of salida.split('\n')) {
      const m = linea.match(/^R\d*\t(.+)\t(.+)$/);
      if (m && m[2] === archivo) return m[1];
    }
  }
  return null;
}
...
export function capturar(repoPath, archivo, { lineStart, lineEnd } = {}) {
  ...
  // Restringir el diff a una ruta apaga la detección de renombrados de git, que
  // necesita comparar el árbol entero. Sin esto, un archivo renombrado se
  // captura como código nuevo y se pierde su "antes". Se busca el nombre viejo
  // aparte y se le pasa al diff junto al nuevo.
  const viejo = nombreAnterior(repoPath, archivo);
  const rutas = viejo ? [viejo, archivo] : [archivo];

  const intentos = [
    { args: [...opciones, '-M', 'HEAD', '--', ...rutas], origen: 'sin commitear' },
    { args: [...opciones, '-M', 'HEAD~1', 'HEAD', '--', ...rutas], origen: 'último commit' },
  ];
  ...
  for (const intento of intentos) {
    ...
    // Un diff de binario no trae ningún @@: git dice "Binary files ... differ"
    // y ya está. Sin esto, capturar() devolvía null y el cambio no se podía
    // registrar de ninguna forma sensata — el "after" de una imagen no se
    // teclea. Se deja constancia de que cambió y de cuánto ocupa.
    if (/^Binary files .* differ$/m.test(diff) || /^GIT binary patch$/m.test(diff)) {
      let bytes = null;
      try { bytes = statSync(join(repoPath, archivo)).size; } catch { /* borrado */ }
      return {
        before: null,
        after: `(archivo binario${bytes == null ? ' borrado' : `, ${bytes.toLocaleString('es-ES')} bytes`})`,
        origen: `${intento.origen}, binario`,
      };
    }
    ...
    // Un cambio contiguo grande —reescribir una función entera, reformatear un
    // bloque— es UN solo tramo, así que el bucle de arriba no lo tocaba y el
    // tope no se aplicaba a lo que más lo necesita.
    if (tramos.length === 1) {
      const t = tramos[0];
      const cabe = Math.max(20, Math.floor(MAX_LINEAS / 2));
      if (t.antes.length > cabe) { t.antes = t.antes.slice(0, cabe); recortado = true; }
      if (t.despues.length > cabe) { t.despues = t.despues.slice(0, cabe); recortado = true; }
    }
