...
export function prompt(codigo, { file, language, contexto } = {}) {
  ...
  return [
    ...
    '- "que" dice QUÉ hace esa línea, en una o dos frases. Concreto: nombra las',
    '  variables y funciones que aparecen, no describas en abstracto.',
    '- Salta las líneas que no explican nada por sí solas: una llave de cierre, una',
    '  línea en blanco, un import obvio. Es mejor no decir nada que decir "cierra el',
    '  bloque".',
    '- Si una línea solo se entiende junto a las de al lado, dilo en la primera de',
    '  ellas y no repitas lo mismo en las demás.',
...
function lanzar({ motor, modelo, prompt: texto }) {
  return new Promise((resolve, reject) => {
    ...
    const hijo = spawn(bin, args, { cwd: neutro, stdio: ['pipe', 'pipe', 'pipe'] });
    let salida = '';
    let error = '';
